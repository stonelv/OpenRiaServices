...
namespace OpenRiaServices.Hosting.AspNetCore.Serialization
{
    internal static class JsonSerializationHelper
    {
        ...
        public static JsonSerializerOptions GetSerializerOptions(DomainServiceDescription domainServiceDescription)
        {
            return s_optionsCache.GetOrAdd(domainServiceDescription.DomainServiceType, _ =>
            {
                var resolver = new DataContractJsonTypeInfoResolver(domainServiceDescription);
            ...
            return s_dataMemberCache.GetOrAdd(type, t =>
            {
                ...
                foreach (var member in metaType.DataMembers)
                {
                    ...
                    if (propInfo != null)
                    {
                        ...
                        var dataContractAttr = t.GetCustomAttribute<DataContractAttribute>();

                        if (dataContractAttr == null || dataMemberAttr != null)
                        {
    ...
    internal class DataContractJsonTypeInfoResolver : DefaultJsonTypeInfoResolver
    {
        ...
        public DataContractJsonTypeInfoResolver(DomainServiceDescription domainServiceDescription)
        {
            ...
            foreach (var complexType in domainServiceDescription.ComplexTypes)
            {
                _knownTypes.Add(complexType);
            }
        }
    ...
    internal class ChangeSetEntryJsonConverter : JsonConverter<ChangeSetEntry>
    {
        public override ChangeSetEntry Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            ...
            object? storeEntity = null;
            DomainOperation operation = DomainOperation.None;
                ...
                if (reader.TokenType == JsonTokenType.PropertyName)
                {
                    ...
                    switch (propertyName)
                    {
                        case "Id":
                            ...
                            break;
                        case "Entity":
                            entity = JsonSerializer.Deserialize<object>(ref reader, options);
                            break;
                        case "OriginalEntity":
                            originalEntity = JsonSerializer.Deserialize<object>(ref reader, options);
                            break;
                        case "StoreEntity":
                            storeEntity = JsonSerializer.Deserialize<object>(ref reader, options);
                            break;
                        case "Operation":
                            operation = JsonSerializer.Deserialize<DomainOperation>(ref reader, options);
                            break;
            ...
            var entry = new ChangeSetEntry(id, entity ?? new object(), originalEntity, operation);
